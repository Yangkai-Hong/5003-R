...
#####Initialize the return lists, and dataframes
  ...
  for (i in seq(1:groups)){
      ...
      xgb.grid <- expand.grid(
        nrounds = 10,
        max_depth = 4,
        eta = 0.02,  #c(0.001, 0.01, 0.05, 0.2) more generalize for mtor was 0.02 akt 759
        gamma = 0,
        colsample_bytree = 0.5,  # c(0.4, 0.6, 0.8), akt 759 0.5
        min_child_weight = 1,
        subsample = 0.6  # was 0.6
      )
...
preodicted_result <- function(df, cols, classname, model_list) {
  ...
  predict_final <- predicted %>% mutate(avg_prob = rowMeans(.[3:52])) %>%
    select(identifier, class, avg_prob)
  # mutate(class_pred = avg_prob > 0.5)
  class_vote <- predicted %>% select(3:52) %>% as.matrix(.) > 0.5
  class_prob <- rowMeans(class_vote)
  class_vote <- rowMeans(class_vote) > 0.5
  predict_final <- predict_final %>% bind_cols(class_prob = class_prob, class_pred = class_vote)
}
########################################################
########################################################
...
combine_pred <- function(base, df1, df2, df3) {
    ...
    rename(Full.model.predict = class_prob.x, Motif.predict = class_prob.y,
    ...
    rename(Phosphoproteome.predict = class_prob, result_pp = class_pred) %>% 
    select(-starts_with("class"), -starts_with("avg_")) %>% 
...
Pengyi Yang, Wei Liu, Jean Yang. (2017). Positive unlabeled learning via wrapper-based adaptive sampling. Proceedings of the Twenty-Sixth International Joint Conference on Artificial Intelligence, Pages 3273-3279.
