...
new_data <- new_data[,-exclude_columns]

```

##Predict Akt & mTOR
```{r}
if (file.exists("prob_result.csv")) {
  prob.result <- read.csv("prob_result.csv")
} else {
  for (vari in c(0.7,0.8,0.9,0.95)) {
    for (ft in c(0.6,0.7,0.8,0.9,1)) {
      labels[paste("Akt", vari,ft, sep = "-")] <- preprocess.fit.predict.evaluate(new_data,labels$Akt,vari, percentage.features = ft)
      labels[paste("mTOR", vari,ft, sep = "-")] <- preprocess.fit.predict.evaluate(new_data,labels$mTOR,vari, percentage.features = ft)
    }
  }
